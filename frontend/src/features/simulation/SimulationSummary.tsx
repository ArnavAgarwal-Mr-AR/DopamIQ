...
import type { ViewMode } from "../simulation/BehavioralForecastGraph";
...
const VIEW_META: Record<ViewMode, {
  ...
  accentHex: string;     // for inline gradient
  badge: string;
}> = {
  day: {
...
const SimulationSummary: React.FC<Props> = ({ summary, view }) => {
  ...
  return (
    <div className="mt-12 bg-white/[0.02] border border-white/5 rounded-3xl p-10 relative overflow-hidden group">
      {/* Top accent line changes per view */}
      <div
      ...
      {/* Context blurb explaining what the graph means */}
      <p className="text-[10px] text-gray-600 uppercase tracking-widest font-black mb-6 border-l-2 pl-3"
