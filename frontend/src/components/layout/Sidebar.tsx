...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex w-64 h-screen bg-black border-r border-white/5 flex-col p-6 fixed left-0 top-0 overflow-y-auto z-50">
        {/* Header / Logo Area */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-black/80 backdrop-blur-xl border-t border-white/10 flex justify-around items-center px-4 py-3 z-[100]">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path;
          return (
            <Link 
              key={item.path}
              to={item.path} 
              className={`flex flex-col items-center gap-1 transition-all ${isActive ? "text-blue-500 scale-110" : "text-gray-600"}`}
            >
              <div className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-blue-500" : "bg-gray-800"}`} />
              <span className="text-[8px] font-black uppercase tracking-tighter italic">{item.label.replace('The ', '')}</span>
            </Link>
          );
        })}
        <button 
          onClick={() => { localStorage.clear(); window.location.href = '/upload'; }}
          className="flex flex-col items-center gap-1 text-red-900/40"
        >
          <div className="w-1.5 h-1.5 rounded-full bg-red-900/40" />
          <span className="text-[8px] font-black uppercase tracking-tighter italic">Detach</span>
        </button>
      </div>
    </>
  );
